...
public class NameTags extends Module {
    ...
    private void drawSpawnerNameTag(DrawContext context) {
        for (BlockEntity blockEntity : StorageEsp.getBlockEntities()) {
            if (blockEntity instanceof MobSpawnerBlockEntity spawner) {
                ...
                if (spawner.getLogic() == null || spawner.getLogic().getRenderedEntity(mc.world, mc.world.getRandom(),  spawner.getPos()) == null)
                    continue;
                String final_string = spawner.getLogic().getRenderedEntity(mc.world, mc.world.getRandom(),spawner.getPos()).getName().getString() + " " + String.format("%.1f", ((float) spawner.getLogic().spawnDelay / 20f)) + "s";
                ...
                if (spawner.getLogic().getRotation() == spawner.getLogic().getLastRotation() && spawner.getLogic().getRotation() == 0f && (float) spawner.getLogic().spawnDelay / 20f == 1f)
                    final_string = spawner.getLogic().getRenderedEntity(mc.world, mc.world.getRandom(),spawner.getPos()).getName().getString() + " loot!";
    ...
    public float getHealth(PlayerEntity ent) {
        // TODO: Fix this and the other getHealth in TargetHud, someday
