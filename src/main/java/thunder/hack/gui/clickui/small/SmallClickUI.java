...
public class SmallClickUI extends Screen {
    ...
    private static SmallClickUI INSTANCE = new SmallClickUI();

    public static SmallClickUI getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new SmallClickUI();
        }
        return INSTANCE;
    }

    public static SmallClickUI getClickGui() {
        return SmallClickUI.getInstance();
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!Objects.equals(currentDescription, "") && ModuleManager.clickGui.descriptions.getValue()) {
            Render2DEngine.drawRoundDoubleColor(context.getMatrices(), mouseX + 7, mouseY + 5, FontRenderers.settings.getStringWidth(currentDescription) + 6, 11, 3f, ClickGui.getInstance().getColor(200), ClickGui.getInstance().getColor(0));
            Render2DEngine.drawRound(context.getMatrices(), mouseX + 7.5f, mouseY + 5.5f, FontRenderers.settings.getStringWidth(currentDescription) + 5, 10, 3f, ClickGui.getInstance().plateColor.getValue().getColorObject());
            FontRenderers.settings.drawString(context.getMatrices(), currentDescription, mouseX + 10, mouseY + 10, -1);
            currentDescription = "";
        }
    }
